...
class Commercial extends CI_Controller
{
	...
	public function customer_list()
	{
		...
		$user_list = $this->user_mod->user_list_db();

		$created_by_list = [];
		foreach ($user_list as $res) {
			$created_by_list[$res['id']] = $res['name'];
		}

		$data['user'] = $created_by_list;
		...
		if (empty($file)) {
			...
			if (strtolower(end($ext)) === 'csv' && $_FILES['upload_excel']['size'] > 0) {
				...
				while (($row = fgetcsv($handle, 2048, $delimiter))) {
					...
					$id_customer = $this->branch_mod->table_rate_domestic_create_process_db($data);
				}
